...
local function get_target_index()
    ...
    if (target == nil or target:GetTargetName() == '') then
        return 0;
    end    
...
local change_handlers = {
    ...
    ['target'] = function() return get_target_index() end,
}
...
-- windower.ffxi.get_items = function()
--     return {
--         safe: {
--             count: int,
--             status: int,
...
--    linkshell_slot: int
    ...
    res['main_job_level'] = AshitaCore:GetDataManager():GetPlayer():GetMainJobLevel() or 1
...
--    status_id: int
    res['index'] = entity.TargetIndex
    res['sub_job'] = jobs[AshitaCore:GetDataManager():GetPlayer():GetSubJob()]
