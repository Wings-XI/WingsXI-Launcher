...
windower.register_event('incoming chunk',function (id,original,modified,is_injected,is_blocked)
    ...
    local previous_message_hash = nil
------- ACTION MESSAGE -------    
    if id == 0x29 then
        local am = {}
        ...
        local current_message_hash = am.actor_id .. '#' .. am.target_id .. '#' .. am.message_id

        if current_message_hash == previous_message_hash then return true end
        ...
        elseif passed_messages:contains(am.message_id) then
            ...
            local outstr = (res.action_messages[am.message_id][language]
                ...
                :gsub('$\123lb\125','\7'))
            windower.add_to_chat(res.action_messages[am.message_id]['color'],make_timestamp(outstr))
            am.message_id = false
...
function multi_packet(...)
    ...
    windower.add_to_chat(res.action_messages[multi_msg[ind]].color,make_timestamp(outstr))
    multi_targs[ind] = nil
    multi_msg[ind] = nil
    multi_actor[ind] = nil
end