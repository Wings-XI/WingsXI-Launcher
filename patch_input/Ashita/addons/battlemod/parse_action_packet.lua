...
--                if m.message == 93 or m.message == 273 then m.status=color_it('Vanish',color_arr['statuscol']) end
            ...
            --        m.simp_name = m.simp_name..' (No Effect)'
                ...
                windower.add_to_chat(color,prefix..make_timestamp(make_condensedamage_number(m.number)..((msg or tostring(m.message))
...
--                    windower.add_to_chat(color, 'act.action==nil : '..m.message..' - '..m.add_effect_message..' - '..msg)
                    ...
                    windower.add_to_chat(color,make_timestamp(make_condensedamage_number(m.add_effect_number)..(msg
            ...
            if m.has_spike_effect and m.spike_effect_message ~= 0 and spike_effect_valid[act.category] then
                ...
                windower.add_to_chat(color,make_timestamp(make_condensedamage_number(m.spike_effect_number)..(msg
...
function get_spell(act)
    ...
    if #act.targets > 0 and #act.targets[1].actions > 0 then
        ...
        spell.name = 'PARSE_FAIL'
        print('battlemod - Category -> ' .. act.category)
    end
