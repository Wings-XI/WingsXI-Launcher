...
debuffs = {
    [2] = S{253,259,273,274,377,471,584,598,678}, --Sleep
...
function update_box()
    ...
    if target and target.valid_target and target.is_npc and (target.claim_id ~= 0 or target.spawn_type == 16) then
        ...
        current_string = 'Debuffs ['..target.id..']\n'
    ...
    if act.category == 4 then
        for i, v in pairs(act.targets) do
            ...
            elseif T{236,237,266,267,268,269,270,271,272,277,278,279,280}:contains(act.targets[i].actions[1].message) then
				...
				elseif T{376,463}:contains(spell) then -- Lullaby
					duration = os.clock() + 30
                elseif T{253,273}:contains(spell) then -- Sleep and sleepga
                    duration = os.clock() + 60
                    if debuffed_mobs[target] and (debuffed_mobs[target][2] or debuffed_mobs[target][19]) then
				...
				elseif T{98,259,274}:contains(spell) then -- Repose and Sleep II and Sleepga II
					...
					if debuffed_mobs[target] then
						...
						if debuffed_mobs[target][193] then
    ...
    elseif T{204,206}:contains(arr.message_id) then
        if debuffed_mobs[arr.target_id] then
            if arr.message_id == 206 then
                ...
                elseif T{2,19,193}:contains(arr.param_1) then
